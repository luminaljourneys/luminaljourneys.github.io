  ...
  async fetch(request, env) {
    ...
    try {
      ...
      const adminRes  = await sendEmail(key, {
        ...
        subject: `Luminal Journeys: New Intake Has Arrived — ${data.firstName} ${data.lastName}`,
...
async function sendEmail(apiKey, payload) {
  const res = await fetch(POSTMARK_API, {
    ...
    body: JSON.stringify({
      ...
      // Logo embedded as an inline CID attachment so it renders in Gmail
      // (which strips data: URIs). Referenced in emailHeader() via src="cid:luminal-logo".
      Attachments: [
        {
          Name:        'logo-email-80.png',
          Content:     LOGO_B64.split(',')[1],
          ContentType: 'image/png',
          ContentID:   `cid:${LOGO_CONTENT_ID}`,
        },
      ],
    }),
  });
...
// Logo mark: 80×80px PNG resized from the 1254×1254 source, base64-encoded.
// Embedded as a CID inline attachment (see sendEmail), referenced via src="cid:luminal-logo".
// NOT a data: URI — Gmail (web + mobile) strips <img src="data:..."> entirely, so the logo
// never renders there. CID attachments render across Gmail, Apple Mail, and Outlook.
// 80px = 40px display (2× retina). ~8.7KB base64 — well within email size limits.
const LOGO_CONTENT_ID = 'luminal-logo';
...
// Shared branded header: dark green bar — logo mark + vertical rule + wordmark,
// mirroring the nav layout: <img> | <div rule> | <span wordmark>
// Georgia font for the wordmark (DM Sans pending brand approval).
function emailHeader() {
  return `
  <tr>
    <td style="background:#172f2d;padding:20px 32px;text-align:center;">
      <!--[if mso]><table role="presentation" align="center"><tr><td><![endif]-->
